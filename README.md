...
(Emphasis mine. There's similar verbiage in C++ standards). 
...
The compiler, unsurprisingly, fails to optimise out the divisions, and emits a bunch of branches and division operations. 
...
I get it, the idea here is to start arguing that in principle there could exist a hypothetical computer that has only saturating operations or where saturating operations are faster. That seems rather silly; how would you go about implementing longer integers on that computer? Or C's unsigned numbers? A saturating adder is nothing more than a normal adder with an extra circuit for saturation, so you'd just implement a way to disable that circuit.
